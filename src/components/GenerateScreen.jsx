...
import { generateResponse, getUsage } from "../api";
...
export default function GenerateScreen({
  ...
  const [usage, setUsage] = useState(null); // Se carga al montar
  const [loadingUsage, setLoadingUsage] = useState(true);
  const [showContexto, setShowContexto] = useState(false);

  // Cargar usage inicial al montar el componente
  useEffect(() => {
    async function loadUsage() {
      try {
        ...
        if (err.message === "TOKEN_EXPIRED") {
          ...
          // Si falla, inicializar con valores default
  ...
  const profileIncomplete = (() => {
    const p = profile || {};
    const filled =
      [p.nombre, p.rubro, p.descripcion, p.horarios, p.pagos].filter(
        (v) => typeof v === "string" && v.trim().length > 0,
      ).length +
      (Array.isArray(p.preguntas_frecuentes) &&
      p.preguntas_frecuentes.length > 0
        ? 1
        : 0) +
  ...
  async function handleGenerate(e) {
    ...
    setCopied(false);
    try {
      const data = await generateResponse(token, {
      ...
      if (err.message === "TOKEN_EXPIRED") {
        onTokenExpired();
      } else if (err.message === "perfil_requerido") {
        onEditProfile();
      } else if (err.message.toLowerCase().includes("límite")) {
        setError(
          "Usaste todas tus generaciones del piloto. Escribinos por WhatsApp para continuar.",
        );
      } else {
  ...
  const canGenerate =
    !loading && mensaje.trim().length > 0 && usage && usage.remaining > 0;

  // Mostrar skeleton mientras carga el usage inicial
  if (loadingUsage) {
    return (
      <div
      ...
      >
        <div className="text-center">
          <div className="inline-block w-12 h-12 border-4 border-[#00D66C] border-t-transparent rounded-full animate-spin mb-4"></div>
          <p className="text-[#808080] text-sm">Cargando...</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span className="text-[#808080] text-xs hidden sm:block">
              {user.name}
            </span>
            <button
              onClick={onLogout}
      ...
      <main
        id="main-content"
        className="max-w-2xl mx-auto px-4 py-6 md:px-6 space-y-4 pb-20"
      >
        {/* ── Profile card ── */}
        <div
        ...
        >
          ...
          >
            {profile?.descripcion ||
              profile?.rubro ||
              "Completá tu perfil para mejores respuestas"}
          </p>
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-right text-xs mt-1" style={{ color: "#808080" }}>
              {mensaje.length}/500
            </p>
          </div>
          ...
          >
            ...
            <span className="font-medium">
              {showContexto
                ? "Ocultar contexto adicional"
                : "Agregar contexto del momento"}
            </span>
            {!showContexto && (
          ...
          {showContexto && (
            <div>
              ...
              <p className="text-xs mb-2" style={{ color: "#808080" }}>
                Stock actual, precios de hoy, promociones. La IA lo prioriza
                sobre todo.
              </p>
              <textarea
                id="gen-contexto"
          ...
          <button
            ...
            style={{
              ...
              boxShadow: canGenerate
                ? "0 6px 20px rgba(0, 214, 108, 0.35)"
                : "none",
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow =
                "0 8px 24px rgba(0, 214, 108, 0.45)";
            }}
            onMouseLeave={(e) => {
        ...
        {respuesta && (
          <div
            ...
            style={{
              background:
                "linear-gradient(to bottom, rgba(0, 214, 108, 0.08) 0%, rgba(0, 214, 108, 0.02) 100%)",
