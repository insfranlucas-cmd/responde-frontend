...
const TONO_OPTS = [
  ...
  { val: "cercano", label: "Cercano" },
  { val: "muy_informal", label: "Muy informal" },
];
...
function Badge({ f, t }) {
  ...
  const cls =
    pct === 100 ? "text-brand" : pct > 50 ? "text-yellow-400" : "text-zinc-500";
  return (
  ...
  return (
    <div>
      ...
      >
        ...
        {hint && (
          <span className="text-zinc-600 font-normal ml-1">({hint})</span>
        )}
      </label>
      {children}
    </div>
    ...
    <div className="space-y-2">
      {items.map((it, i) => (
        <div key={i} className="flex gap-2 items-start">
          ...
          <button
            type="button"
            onClick={() => remove(i)}
            className="text-zinc-600 hover:text-red-400 text-lg leading-none pt-2 transition-colors"
          >
            ×
          </button>
        </div>
      ...
      {items.length < 10 && (
        <button
          type="button"
          onClick={add}
          className="text-brand text-sm hover:text-brand-dark transition-colors"
        >
          + Agregar producto
        </button>
      )}
...
function QuickSetupScreen({ form, set, loading, error, onSave, onFullForm }) {
  return (
    <main
      id="main-content"
      className="min-h-screen flex flex-col px-4 py-6 max-w-lg mx-auto w-full"
    >
      ...
      <div className="mb-6">
        <h2 className="text-white text-xl font-bold mb-2">
          ⚡ La IA necesita saber esto
        </h2>
        <p className="text-zinc-400 text-sm leading-relaxed">
          Solo 2 datos para tu primera respuesta.
          <br />
      ...
      <form onSubmit={onSave} className="space-y-4">
        ...
        <Field label="Rubro" required htmlFor="qs-rubro">
          <input
            ...
            placeholder="Ej: Venta mayorista de insumos de limpieza"
            maxLength={150}
            className={inputCls}
          />
        </Field>
        ...
        <div className="pt-2 space-y-3">
          ...
          >
            {loading
              ? "Guardando..."
              : "Guardar y generar mi primera respuesta →"}
          </button>
...
export default function ProfileScreen({
  ...
  const [form, setForm] = useState({
    ...
    nombre_titular: ip.nombre_titular || "",
    // B
    whatsapp: ip.whatsapp || "",
    telefono: ip.telefono || "",
    telefono_alt: ip.telefono_alt || "",
    email: ip.email || "",
    direccion: ip.direccion || "",
    maps: ip.maps || "",
    sitio_web: ip.sitio_web || "",
    instagram: ip.instagram || "",
    facebook: ip.facebook || "",
    // C
    horarios: ip.horarios || "",
    pagos: ip.pagos || "",
    politica_entregas: ip.politica_entregas || "",
    delivery_activo: ip.delivery_activo ?? false,
    delivery_zona: ip.delivery_zona || "",
    delivery_tiempo: ip.delivery_tiempo || "",
    emite_factura: ip.emite_factura ?? false,
    ruc: ip.ruc || "",
    tiempo_respuesta: ip.tiempo_respuesta || "",
    condiciones_mayorista: ip.condiciones_mayorista || "",
    devoluciones: ip.devoluciones || "",
    garantia: ip.garantia || "",
    // D
    marcas: ip.marcas || "",
    // E
    tono_respuesta: ip.tono_respuesta || "cercano",
    usar_emojis: ip.usar_emojis ?? true,
    restricciones: ip.restricciones || "",
    mensaje_bienvenida: ip.mensaje_bienvenida || "",
    // legacy
    nombre_encargado: ip.nombre_encargado || "",
    frase_diferencial: ip.frase_diferencial || "",
    objeciones: ip.objeciones || "",
    descuentos: ip.descuentos || "",
    productos: ip.productos || "",
    precios: ip.precios || "",
  });

  const [items, setItems] = useState(Array.isArray(ip.items) ? ip.items : []);
  ...
  const [showQuickSetup, setShowQuickSetup] = useState(!initialProfile);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const faqsLimpios = faqs.filter(
        (f) => f.pregunta.trim() !== "" || f.respuesta.trim() !== "",
      );
      const payload = {
        ...form,
        items: itemsLimpios,
        preguntas_frecuentes: faqsLimpios,
      };
  ...
  const progColor =
    progress < 40 ? "bg-red-500" : progress < 70 ? "bg-yellow-500" : "bg-brand";

  // Section badges
  const bA = sectionProgress([
    form.nombre,
    form.rubro,
    form.descripcion,
    form.nombre_titular,
  ...
  const bC = sectionProgress([
    form.pagos,
    form.politica_entregas,
    form.delivery_activo,
    form.tiempo_respuesta,
  ]);
  const bD = sectionProgress([
    items.length > 0 || form.marcas,
    form.marcas,
    faqs.length > 0,
  ]);
  const bE = sectionProgress([
    form.tono_respuesta,
    form.restricciones,
    ...
    >
      ...
      <div className="mb-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="w-full bg-zinc-800 rounded-full h-1.5">
            <div
              className={`h-1.5 rounded-full transition-all duration-300 ${progColor}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-1">
            ...
            <div className="pb-4 space-y-4">
              ...
              <div className="pt-3 space-y-4">
                ...
                <Field
                  label="Descripción breve"
                  hint="máx 300 chars"
                  htmlFor="profile-descripcion"
                >
                  <textarea
                    id="profile-descripcion"
                    value={form.descripcion}
                    onChange={(e) => set("descripcion", e.target.value)}
                    placeholder={
                      "Contá brevemente qué hace tu negocio y qué lo diferencia.\nEj: Distribuidora de productos de limpieza para hogares y comercios, con más de 10 años en el mercado."
                    }
                    rows={3}
                    maxLength={300}
                    className={textareaCls}
                ...
                <Field
                  label="Nombre del titular / persona de contacto"
                  htmlFor="profile-nombre-titular"
                >
                  <input
                    id="profile-nombre-titular"
                    type="text"
                ...
                <Field label="Horarios de atención" htmlFor="profile-horarios">
                  <input
                    ...
                    placeholder="Lun-Vie 7:30-17:30 / Sáb 8:00-12:00 / Dom cerrado"
                    maxLength={150}
                    className={inputCls}
                  />
                </Field>
                <Field label="¿Emitís factura?" htmlFor="profile-emite-factura">
                  <Toggle
                    id="profile-emite-factura"
                    value={form.emite_factura}
                    onChange={(v) => set("emite_factura", v)}
                    label={form.emite_factura ? "Sí" : "No"}
                  />
                  {form.emite_factura && (
                    <input
                      type="text"
              ...
              <div className="pt-3 space-y-4">
                ...
                <Field label="Dirección exacta" htmlFor="profile-direccion">
                  <input
                    ...
                    placeholder="Av. Mcal. López 2345 c/ Perú, Barrio Jara, Asunción"
                    maxLength={300}
                    className={inputCls}
                  />
                </Field>
                ...
                <Field
                  ...
                  hint="opcional"
                  htmlFor="profile-telefono-alt"
                >
                  <input
                    id="profile-telefono-alt"
                    type="text"
                ...
                <Field
                  label="Sitio web"
                  hint="opcional"
                  htmlFor="profile-sitio-web"
                >
                  <input
                    id="profile-sitio-web"
                    type="text"
            ...
            <div className="pb-4 space-y-4">
              ...
              <div className="pt-3 space-y-4">
                ...
                <Field
                  ...
                  htmlFor="profile-politica-entregas"
                >
                  <textarea
                    id="profile-politica-entregas"
                    value={form.politica_entregas}
                    onChange={(e) => set("politica_entregas", e.target.value)}
                    placeholder="¿Entregan? Zonas, costo, mínimo de compra, tiempo..."
                    rows={3}
                    maxLength={400}
                    className={textareaCls}
                  />
                ...
                <Field
                  label="Hacemos delivery"
                  htmlFor="profile-delivery-activo"
                >
                  <Toggle
                    id="profile-delivery-activo"
                  ...
                  {form.delivery_activo && (
                    <div className="mt-3 space-y-3">
                      <input
                        type="text"
                        value={form.delivery_zona}
                        onChange={(e) => set("delivery_zona", e.target.value)}
                      ...
                      <input
                        type="text"
                        value={form.delivery_tiempo}
                        onChange={(e) => set("delivery_tiempo", e.target.value)}
                        placeholder="Tiempo estimado (ej: 24-48hs)"
                        maxLength={100}
                        className={inputCls}
                      />
                    </div>
                ...
                <Field
                  label="Política de cambios / devoluciones"
                  hint="opcional"
                  htmlFor="profile-devoluciones"
                >
                  <textarea
                    id="profile-devoluciones"
                    value={form.devoluciones}
                    onChange={(e) => set("devoluciones", e.target.value)}
                    placeholder="Ej: Cambios dentro de los 7 días con comprobante"
                    rows={2}
                    maxLength={300}
                    className={textareaCls}
                  />
                ...
                <Field
                  ...
                  htmlFor="profile-tiempo-respuesta"
                >
                  <input
                    id="profile-tiempo-respuesta"
                    type="text"
                    value={form.tiempo_respuesta}
                    onChange={(e) => set("tiempo_respuesta", e.target.value)}
                    placeholder="Ej: menos de 1 hora en horario de atención"
                    maxLength={100}
                    className={inputCls}
                  />
                </Field>
                <Field
                  label="Condiciones para cuenta mayorista"
                  hint="opcional"
                  htmlFor="profile-condiciones-mayorista"
                >
                  <textarea
                    id="profile-condiciones-mayorista"
                    value={form.condiciones_mayorista}
                    onChange={(e) =>
                      set("condiciones_mayorista", e.target.value)
                    }
                    placeholder="Ej: Mínimo 10 unidades, precio especial desde..."
                    rows={2}
                    maxLength={300}
                    className={textareaCls}
                  />
              ...
              <div className="pt-3 space-y-4">
                <Field label="Productos con precio (hasta 10)">
                  <p className="text-zinc-500 text-xs mb-3">
                    Solo los más consultados o con precio fijo. No hace falta
                ...
                <Field
                  ...
                  hint="opcional"
                  htmlFor="profile-marcas"
                >
                  <input
                    id="profile-marcas"
                    type="text"
                    value={form.marcas}
                    onChange={(e) => set("marcas", e.target.value)}
                    placeholder="Marcas que manejás, separadas por coma. Ej: Cavallaro, Mr. Músculo, Mar, Magistral"
                    maxLength={200}
                    className={inputCls}
                  />
                </Field>
                <Field label="Preguntas frecuentes (hasta 5)">
                  <p className="text-zinc-500 text-xs mb-3">
                    Las preguntas que más te hacen y cómo las respondés. Son las
                    respuestas más precisas que va a dar la IA.
                  </p>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 overflow-hidden">
          <SectionHeader
            title="E — Configuración de respuestas"
            badge={bE}
            open={open.E}
            toggle={() => toggle("E")}
            categoryBadge={
          ...
          {open.E && (
            <div className="pb-4 space-y-4">
              ...
              <div className="pt-3 space-y-4">
                  ...
                  <div className="flex gap-2">
                    {TONO_OPTS.map((o) => (
                      <button
                        key={o.val}
                        type="button"
                        onClick={() => set("tono_respuesta", o.val)}
                        className={`flex-1 py-2 rounded-xl text-xs font-medium border transition-colors
                          ${
                            form.tono_respuesta === o.val
                              ? "bg-brand text-black border-brand"
                              : "bg-zinc-800 text-zinc-400 border-zinc-700 hover:border-zinc-500"
                          }`}
                      >
                ...
                <Field
                  label="¿Usar emojis en las respuestas?"
                  htmlFor="profile-usar-emojis"
                >
                  <Toggle
                    id="profile-usar-emojis"
                    value={form.usar_emojis}
                    onChange={(v) => set("usar_emojis", v)}
                    label={form.usar_emojis ? "Sí" : "No"}
                  />
                </Field>
                <Field
                  label="Restricciones o excepciones"
                  hint="opcional"
                  htmlFor="profile-restricciones"
                >
                  <textarea
                ...
                <Field
                  label="Mensaje de bienvenida personalizado"
                  hint="opcional"
                  htmlFor="profile-mensaje-bienvenida"
                >
                  <textarea
                    id="profile-mensaje-bienvenida"
      ...
      <ReportButton userEmail={user.email} />
    </main>
  );
}
